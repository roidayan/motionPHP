...
(function($){
    $.fn.flipbook = function(options){
        options = $.extend({
            ...
            'preload': 100, //the percentage of frames to load before playing
        ...
        return this.each(function(){
            ...
            var preload = $image.attr('preload')||options.preload;
            var numberOfFramesToLoadBeforePlaying = (end+1-start)*(preload/100);
            //check if the image have been preloaded,
            //if they have start the animation
            function shouldStartAnimation(){
                preloadCount += step;
                if(!isPlaying && preloadCount >= numberOfFramesToLoadBeforePlaying){
                    setTimeout(flipImage, holdTime);
                    isPlaying = true;
                }      
            }
